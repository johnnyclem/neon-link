...
// The session timeline for a standalone (no Link yet) Teensy: tempo from
// the performance encoder, transport from its push switch. Produces the
// same TimelineSnapshot the ESP32 target publishes from Link, so the
// pulse engine and the UI phase bar are driven identically — when the
// Link port lands (native Ethernet + QNEthernet), only the writer of
