#!/usr/bin/env bash
# Flash link-sync to a Waveshare ESP32-S3-RLCD-4.2 (N16R8).
#
...
if [[ -z "$PORT" ]]; then
  echo "No /dev/cu.usbmodem* found. Plug in the RLCD USB-C cable." >&2
  exit 1
fi

BUILD_DIR="build-linksync-rlcd"
SDKCONFIG="$BUILD_DIR/sdkconfig"
DEFAULTS="sdkconfig.defaults;sdkconfig.defaults.linksync-rlcd"

need_reconfigure=0
if [[ ! -f "$SDKCONFIG" ]]; then
  need_reconfigure=1
elif ! grep -q 'CONFIG_NEON_BOARD_LINKSYNC_RLCD=y' "$SDKCONFIG"; then
  echo "isolated sdkconfig is not the RLCD board; reconfiguring..."
  need_reconfigure=1
fi
...
echo "Building link-sync RLCD firmware (isolated $BUILD_DIR)..."
"${idf[@]}" build
...
echo "First boot: the panel shows NEON LINK, then the live status face."
