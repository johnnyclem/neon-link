...
const char* kTag = "jc3248";

constexpr int kW = halesp::kAxsW;   // 320
constexpr int kH = halesp::kAxsH;   // 480
constexpr int kCx = kW / 2;
constexpr int kCy = kH / 2;

// RGB565, byte-swapped to GC9A01 wire order (see kAxsSwapBytes).
constexpr uint16_t rgb(uint8_t r, uint8_t g, uint8_t b) {
  const uint16_t c = static_cast<uint16_t>(((r & 0xf8) << 8) |
                                           ((g & 0xfc) << 3) | (b >> 3));
  if (halesp::kAxsSwapBytes) {
...
void apply_backlight(int64_t now_us) {
  ...
  if (eff != g_bl_applied) {
    halesp::lcd_axs15231b_backlight_level(static_cast<uint8_t>(eff));
    g_bl_applied = eff;
  }
}
...
void confirm_choice(bool yes) {
  if (yes) {
    ...
    halesp::lcd_axs15231b_blit(g_fb, 0, 0, kW, kH);
...
void handle_touch() {
  ...
  int x = 0;
  int y = 0;
  const bool down = halesp::lcd_axs15231b_touch_poll(&x, &y);
  if (down) {
...
void jc3248_task(void*) {
  if (!halesp::lcd_axs15231b_init()) {
    ESP_LOGE(kTag, "GC9A01 init failed");
    vTaskDelete(nullptr);
    return;
  }
  // No rotary encoder on this board; touch is the only input. The encoder
  // backend stays uninitialized (take_* return 0/kNone), so handle_encoder
  // is a harmless no-op and the settings helpers stay referenced.
  const bool touch = halesp::lcd_axs15231b_touch_init(kPinI2cSda, kPinI2cScl);
  ...
  text_cx(kCx, kCy + 12, "link-jc", 1, pal.muted);
  halesp::lcd_axs15231b_blit(g_fb, 0, 0, kW, kH);
  g_dimmer.note_activity(esp_timer_get_time());
  apply_backlight(esp_timer_get_time());
  ...
  for (;;) {
    const int64_t now = esp_timer_get_time();
    // BRING-UP: keep the panel awake every frame so it never idle-blanks
    // (touch cannot yet register activity). Remove once touch is verified.
    g_dimmer.note_activity(now);
    // BRING-UP heartbeat: confirm the render loop is actually iterating.
    static unsigned frame_ct = 0;
    if ((frame_ct++ % 60) == 0) {
      ESP_LOGI(kTag, "render loop alive, frame=%u", frame_ct);
    }
    {
    ...
    if (g_dimmer.level(now) == neon::ui::IdleDimmer::Level::kBlank) {
      ...
      int tx = 0;
      int ty = 0;
      const bool tdown = halesp::lcd_axs15231b_touch_poll(&tx, &ty);
    ...
    // BRING-UP DIAG: cycle the whole panel red→green→blue (~0.7 s each),
    // bypassing the UI paint. Proves whether the panel accepts continuous
    // frames (screen cycles colors) vs only the first frame (stuck/blank).
    // Remove once the display is understood.
    {
      static const uint16_t kDiag[3] = {rgb(255, 0, 0), rgb(0, 255, 0),
                                        rgb(0, 0, 255)};
      const uint16_t c = kDiag[(now / 700000) % 3];
...
void neon_start_jc3248_service() {
  xTaskCreatePinnedToCore(jc3248_task, "jc3248", 8192, nullptr, 3, nullptr,
                          kNeonCoreApp);
}
