...
static void xiao_led_task(void *arg)
{
	(void)arg;
	int on = 0;
	for (;;) {
		gpio_set_level(GPIO_NUM_27, on);
		on = !on;
...
void app_main(void)
{
	...
	/* XIAO user LED GPIO27 — hosted slave does not touch it otherwise.
	 * Slow blink = this image is running. */
	{
		gpio_config_t led = {0};
		led.pin_bit_mask = 1ULL << 27;
		led.mode = GPIO_MODE_OUTPUT;
		gpio_config(&led);
		xTaskCreate(xiao_led_task, "xiao_led", 2048, NULL, 1, NULL);
	}
