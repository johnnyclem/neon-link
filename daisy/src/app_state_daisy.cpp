// Daisy implementation of the shared app_state buses
// (components/app_state/include/app_state/*.h — the same headers the
// ESP32 and Teensy firmware compile against). The seqlock types are
// portable; the FreeRTOS queues become interrupt-masked rings, sized
// like their ESP counterparts.
//
// Single-core rule (docs/DAISY.md §6): main-loop code may read the
// seqlock buses freely, but ISRs must not — an ISR spinning on a
// mid-publish seqlock waits forever for a writer that cannot run. The
// audio callback and the MIDI tick ISR read IRQ-masked staging copies
// instead (audio_daisy.cpp, midi_daisy.cpp).
