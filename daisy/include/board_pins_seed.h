...
// GND, internal pullups). ENC1 (index 0) navigates the menu; ENC2
// (index 1) is the performance encoder (tempo / transport). ----------
inline constexpr int kNumEncoders = 2;
inline constexpr daisy::Pin kEncPins[kNumEncoders][3] = {
    {daisy::seed::D0, daisy::seed::D1, daisy::seed::D2},
    {daisy::seed::D3, daisy::seed::D4, daisy::seed::D5},
};

// No standalone buttons on this board (the encoder switches cover it).
inline constexpr int kNumButtons = 0;
inline constexpr daisy::Pin kButtonPins[1] = {daisy::Pin()};  // unused

// --- Pulse outputs. The edge stream uses virtual channel bits 0..5
// (CLK1..4, RESET, RUN — the same scheme as the AMYboard and Teensy
// builds); the timer ISR maps physical pin i to virtual channel
// kPulsePinChannel[i]. Contiguous block so a single header row serves
// the jacks. 3.3 V logic — level-shift to 5 V for Eurorack
// (HARDWARE.md §5). --------------------------------------------------
...
// Non-inverting input conditioning; a pulldown keeps a floating bench
// setup quiet.
